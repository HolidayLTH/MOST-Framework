...
def get_quantiles_loss_and_q_risk(outputs: torch.Tensor,
    ...
    #print(losses_array)
    losses_array = torch.nan_to_num(losses_array, nan=0.0)
    ...
    p = 2
    q_loss_samples = ((losses_array.sum(dim=-1) ** p).mean(dim=-1)) ** (1/p)

    q_loss = q_loss_samples.mean()
    # compute q_risk for each quantile
    # q_risk = 2 * (losses_array.sum(dim=1).sum(dim=0)).mean(dim=0) / (targets.abs().sum().unsqueeze(-1))
    q_risk = 2 * (losses_array.sum(dim=1).sum(dim=0)) / (targets.abs().sum().unsqueeze(-1) + 1e-6) # 每个分位数一个

    return q_loss, q_risk, q_loss_samples
