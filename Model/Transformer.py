...
class Transformer_Benchmark(nn.Module):
    """
    Benchmark Transformer model matching TFT_GAT input/output interfaces.

    - Same embedding pipeline as TFT_GAT via `transform_inputs`.
    - No LSTM, no GAT, no static enrichment.
    - Static features are treated as covariates and concatenated to temporal inputs.
    - Transformer self-attention is non-causal (history can attend to future).
    ...
    def __init__(self, config: DictConfig):
        ...
        self.historical_in_proj = nn.Linear(hist_dim + static_dim, self.state_size)
        self.future_in_proj = nn.Linear(fut_dim + static_dim, self.state_size)
        ...
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=self.lstm_layers)
    ...
    def forward(self, batch, edge_index, target_N, IS_EVAL, IS_STATION, IS_PHYSIC):
        ...
        # =========== Input projections ==============
        historical_input = self.historical_in_proj(historical_input)
        future_input = self.future_in_proj(future_input)

        # =========== Transformer Encoder (non-causal) ==============
        sequence = torch.cat([historical_input, future_input], dim=1)
        encoded = self.transformer(sequence)

        output_sequence_length = self.num_future_steps - self.target_window_start_idx
        future_encoded = encoded[:, self.num_historical_steps + self.target_window_start_idx:, :]

        # =========== output projection for IU ==============
        predicted_quantiles_IU = self.output_layer_IU(future_encoded)
