...
class InputChannelEmbedding(nn.Module):
    ...
    def __init__(self, state_size: int, num_numeric: int, num_categorical: int, categorical_cardinalities: List[int],
        ...
        self.time_distribute = time_distribute #如果是时序变量就为True，否则为False
        ...
        if self.time_distribute:  # 逐时间点操作
...
class CategoricalInputTransformation(nn.Module):
    ...
    def __init__(self, num_inputs: int, state_size: int, cardinalities: List[int]):
        ...
        self.num_inputs = num_inputs
        self.state_size = state_size
        self.cardinalities = cardinalities # 类别数量的list
        ...
        for idx, cardinality in enumerate(self.cardinalities):
            self.categorical_embedding_layers.append(nn.Embedding(cardinality, self.state_size)) # cardinality是类别数量
...
class GAT(nn.Module):
    def __init__(self, input_dim, output_dim, num_heads, dropout=0.0):
        ...
        self.gat_conv = GATConv(
                                ...
                                heads=num_heads,
                                dropout=dropout,
                                concat=False,          # True 表示拼接多头输出
                                negative_slope=0.2,     # LeakyReLU 的负斜率
                                add_self_loops=False
                            )
    def forward(self, hc_bridge, edge_index):
        hc_bridge_gat, (edge_index_attn, alpha) = self.gat_conv(hc_bridge, edge_index,return_attention_weights=True)
        return hc_bridge_gat, edge_index_attn, alpha

#%% --- 单调性模块 --- 
class MonotonicLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=True, positive_fn="softplus"):
        super().__init__()
    ...
    def forward(self, x):
        # 确保权重非负，从而保证输入增大时输出不减小
        positive_weight = self.positive_fn(self.weight)
        y = torch.matmul(x, positive_weight.T)
...
class PINN(nn.Module):
    def __init__(self, num_W_inputs, num_static_station_inputs, num_static_OD_inputs, num_Hour_inputs, num_Day_inputs, state_size, dropout=0.05):
        super().__init__()
        self.state_size = state_size
        self.dropout = dropout
        ...
        self.TT_proj = nn.Sequential(
            MonotonicLinear(1, state_size, bias=False), # TT单调性嵌入
            nn.Tanh()
        )
        
        self.SL_proj = nn.Sequential(
            MonotonicLinear(1, state_size, bias=False), # SL单调性嵌入
            nn.Tanh()
        )

        # ==========================================
        # 1. 基础嵌入适配器：W，O，D，OD，Day
        # ==========================================
        self.W_adapter = nn.Sequential(
            nn.Linear(num_W_inputs * state_size, state_size),
            nn.ReLU(),
            nn.Linear(state_size, state_size) # 保持维度一致，或者映射到物理需要的维度
        )
        self.Hour_adapter = nn.Sequential(
            nn.Linear(num_Hour_inputs  * state_size, state_size),
            nn.ReLU(),
            nn.Linear(state_size, state_size) # 保持维度一致，或者映射到物理需要的维度
        )
        self.Day_adapter = nn.Sequential(
            nn.Linear(num_Day_inputs  * state_size, state_size),
            nn.ReLU(),
            nn.Linear(state_size, state_size) # 保持维度一致，或者映射到物理需要的维度
        )
        self.O_adapter = nn.Sequential(
            nn.Linear(num_static_station_inputs  * state_size, state_size),
            nn.ReLU(),
            nn.Linear(state_size, state_size) # 保持维度一致，或者映射到物理需要的维度
        )
        self.OD_adapter = nn.Sequential(
            nn.Linear(num_static_OD_inputs  * state_size, state_size),
            nn.ReLU(),
            nn.Linear(state_size, state_size) # 保持维度一致，或者映射到物理需要的维度
        )

        # ==========================================
        # 2. 静态上下文编码器: 静态 (O, OD, Day)
        # ==========================================
        self.static_context_grn = GatedResidualNetwork(
            input_dim = 3 * state_size,
        ...
        # ==========================================
        # 3. 动态上下文编码器: 动态(天气 + 流量 + Hour) | 静态 (O, OD, Day)
        # ==========================================
        self.dynamic_ts_selection = VariableSelectionNetwork(
            input_dim = state_size,
            num_inputs = 3, # 天气 + 流量 + Hour
            hidden_dim = state_size,
            dropout = dropout,
            context_dim = state_size # 接受context, 静态 (O, OD, Day)
        )

        # ==========================================
        # 4. FiLM 网络
        # ==========================================
        self.gamma_gen = nn.Linear(state_size, state_size)
        self.beta_gen = MonotonicLinear(state_size, state_size, bias=False)
        self.S_pred = MonotonicLinear(state_size, 1, bias=False) # 输出S的值
        ...
        self.a_net = nn.Sequential(
            nn.Linear(state_size, state_size, bias=False),
            nn.ReLU(),
            nn.Linear(state_size, 1, bias=False) # 输出 Alpha, Beta
        )
        
        # self.b_net = nn.Sequential(
        #     nn.Linear(state_size, state_size),
        #     nn.ReLU(),
        #     nn.Linear(state_size, 1), # 输出 Alpha, Beta
        #     nn.Sigmoid() # 缩放到 0-1
        # )
        
        ## Rb
        self.gamma_gen1 = nn.Linear(state_size, state_size)
        
        self.rb_ts_selection = VariableSelectionNetwork(
            input_dim = state_size,
            num_inputs = 3, # 天气 + OD流量 + Hour
            hidden_dim = state_size,
            dropout = dropout,
            context_dim = state_size # 接受context
        )
        
        self.rb_net = nn.Sequential(
            MonotonicLinear(self.state_size, self.state_size, bias=False),
            nn.ReLU(),
            MonotonicLinear(self.state_size, 1, bias=False),
            nn.Tanh()
        )

    def forward(self, SL_raw, TT_raw, I_pred, I_raw, phi_raw, maxphi, W,  Hour, Hour_raw, Day, O, OD, batch_indices, IS_EVAL):
        """
        SL_raw:    [batch, time, 1 ] - 主信号 (供给损失)
        TT_raw:    [batch, time, 1 ] - 主信号 (出行时间)
        I_raw:     [batch, time, 3 ] - 调制信号 (OD流量,用于测试阶段推断站点流量)
        phi_raw:   [batch, time, 1 ] - 调制信号 (站点流量,用于训练阶段直接输入站点流量)
        W:    [batch, time, num_W * state_size] - 天气
        Hour: [batch, time, 1 * state_size] - 小时信息
        Day: [batch, num_day * state_size] - 天信息
        O:    [batch, num_O * state_size] - 静态
        OD:   [batch, num_OD * state_size] - 静态
        """
        # === A. 计算站点流量 ===
        batch_size, time_steps, num_quantiles = I_pred.shape
        ...
        ## 3.准备 scatter_add 索引 (关键步骤)
        index_phi = inverse_indices.view(-1, 1, 1).expand(-1, time_steps, num_quantiles)
        denominator = torch.zeros(num_origins, time_steps, num_quantiles, device=device)
        denominator.scatter_add_(0, index_phi, I_pred)
        phi_pred = denominator[inverse_indices]
        
        ## 4.广播回填得到站点流量
        if phi_raw is not None:
            phi = (phi_raw + phi_pred[...,[1]])/2
        else:
            phi = phi_pred[...,[1]]
        
        ## 5.得到站点流量嵌入
        phi_emb = self.phi_proj(phi)
        ...
        # === B. 基础嵌入适应 ===
        W_emb = self.W_adapter(W)
        Hour_emb = self.Hour_adapter(Hour)
        Day_emb = self.Day_adapter(Day)
        O_emb = self.O_adapter(O)
        OD_emb = self.OD_adapter(OD)
        
        # === B. 构建OD静态上下文 (Static Context) ===
        static_input = torch.cat([O_emb, OD_emb, Day_emb], dim=-1) # [batch, static_dim]
        c_static = self.static_context_grn(static_input) # [batch, state_size], 通过 GRN 生成静态上下文信号 (c_lsim)
        
        # === C. 准备动态输入 (Dynamic Inputs) ===

        dynamic_input = torch.cat([W_emb, phi_emb, Hour_emb], dim=-1) #!!!!!!!!!! 天气和流量组成的调制信号 
        selected_dynamics,_ = TFT_GAT.apply_temporal_selection(temporal_representation=dynamic_input,
            static_selection_signal=c_static,
            temporal_selection_module=self.dynamic_ts_selection
        ) #[num_samples x num_future_steps x state_size]
 
        # === D. FiLM 调制 S ===
        ## 1.嵌入主信号
        SL_emb = self.SL_proj(torch.abs(SL_raw))  # 1/TT不为0时, 且SL为0时, Rb一定为1, S为最大值 [0,1]
        # TT_emb = torch.relu(self.TT_proj(torch.abs(TT_raw)))     # 1/TT为0时, S一定为0, Rb一定为无穷大 [0,0.2]
        TT_emb = self.TT_proj(torch.abs(TT_raw))     # 1/TT为0时, S一定为0, Rb一定为无穷大 [0,0.2]
        step_mask = (TT_emb != 0).any(dim=-1, keepdim=True).type_as(TT_emb)
        TT_emb_shifted = TT_emb + step_mask 
        
        delta = 1
        epsilon = 1e-3
        log_sl = torch.log(SL_emb / epsilon + delta)
        log_tt = torch.log(TT_emb_shifted / epsilon + epsilon)
        log_diff = torch.relu(torch.exp(log_sl - log_tt)-epsilon)
        
        ## 2.调制OD供给的表征
        gamma = F.softplus(self.gamma_gen(selected_dynamics)) # Condition
        S_emb = self.beta_gen( gamma * log_diff )
        
        ## 3.输出OD供给的值S (需要进行弱监督)
        S = 1 - F.tanh(self.S_pred(S_emb)) # [batch, time, 1], S反比于SupplyLoss
        
        ## 4.汇总站点供给表征
        index_S = inverse_indices.view(-1, 1, 1).expand(-1, time_steps, self.state_size)
        numerator = torch.zeros(num_origins, time_steps, self.state_size, device=device)
        weighted_S = S_emb * I_pred[...,[1]]
        numerator.scatter_add_(0, index_S, weighted_S)
        S_in_grouped = numerator / (denominator[...,[1]] + 1e-6)
        S_in = S_in_grouped[inverse_indices]

        # === 3. PPEM 生成 Alpha, Beta, Eta, Rb ===
        ## 1. 计算Alpha, Beta
        # alpha_O = self.a_net(O_emb) * 9.5 + 0.5 # Alpha: 通常较小 (e.g. 0.15)，必须 > 0
        O_emb_expanded = O_emb.unsqueeze(1).expand(-1, S_in.shape[1], -1)
        # combined_input = torch.cat([S_in, O_emb_expanded], dim=-1)
        alpha_O = F.tanh(torch.abs(self.a_net(O_emb_expanded * S_in))) * 10

        # beta_O = self.b_net(O_emb) * 2 + 1 # Beta: 幂指数，通常 >= 1.0 (线性或超线性增长)
        # alpha_O = torch.full_like(maxphi, 3)
        beta_O = torch.full_like(alpha_O, 2)

        ## 2. 计算 Eta (需要历史最大站流量当辅助)
        eta_O = torch.abs(maxphi.unsqueeze(1))*0.5 + 1e-6
        eta_O = eta_O.expand(-1, time_steps, -1)

        ## 3. 计算 Rb (动态, 受 S, W , Hour, OD影响)
        rb_input = torch.cat([W_emb, I_emb, Hour_emb], dim=-1) #!!!!!!!!!!!!!!!!!!!是S_emb还是I_emb
        selected_rb,_ = TFT_GAT.apply_temporal_selection(temporal_representation=rb_input,
            static_selection_signal=c_static,
            temporal_selection_module=self.rb_ts_selection
        ) #[num_samples x num_future_steps x state_size]
        gamma1 =  F.softplus(self.gamma_gen1(selected_rb))
        rb = self.rb_net(gamma1 * S_emb) * 4 + 1 # [batch, time, 1]
        
        ## 4. 计算 R_physics
        valid_mask = (Hour_raw[...,[0]] > 5).float()
        alpha_O_seq = alpha_O * valid_mask
        beta_O_seq = beta_O
        load_ratio = torch.clamp(phi / eta_O, max=2.0) + 1e-8
        congestion_term =  torch.pow(alpha_O_seq * load_ratio, beta_O_seq)
        congestion_term = torch.clamp(congestion_term, max=4.0) + 1e-8
        r_physics = rb * (1.0 + congestion_term)

        return phi_pred, S, alpha_O_seq, beta_O_seq, eta_O, rb, r_physics, congestion_term

#%% --- 主模型TFT ---
class TFT_GAT(nn.Module):
    """
    Parameters
    ----------
    config: DictConfig
        ...
        inputs are categorical) are specified. 
        Moreover, the configuration mapping should contain a key named ``model``,
    ...
    def __init__(self, config: DictConfig):
        ...
        # -- 历史和未来变量的代表（从分类和连续里面选不为0的那个），会自适应计算历史窗口和未来窗口 -- 
        ...
        # model props
        # ============
        self.task_type = config.task_type #回归还是分类
        self.attention_heads = config.model.attention_heads #注意力头的个数
        self.dropout = config.model.dropout #dropout比例
        self.lstm_layers = config.model.lstm_layers #lstm层数
        ...
        if self.task_type == 'regression':
            ...
            self.num_outputs = len(self.output_quantiles)*self.num_target # 输出数量
            self.num_auxiliary_outputs = len(self.output_quantiles)*self.num_auxiliary_target # 输出数量
        ...
        self.state_size = config.model.state_size #模型的嵌入维度

        # =====================
        # Input Transformation: 嵌入层
        # =====================
        ...
        # =============================
        # Variable Selection Networks: 变量选择
        # =============================
        ...
        self.historical_ts_selection = VariableSelectionNetwork(
            ...
            dropout=self.dropout,
            context_dim=self.state_size) # 接受context
        ...
        # =============================
        # static covariate encoders: 四个不同分工的编码器
        # =============================
        static_covariate_encoder = GatedResidualNetwork(input_dim=self.state_size,
                                                        hidden_dim=self.state_size,
                                                        output_dim=self.state_size,
                                                        dropout=self.dropout)
        self.static_encoder_selection = copy.deepcopy(static_covariate_encoder) # 用来变量选择
        self.static_encoder_enrichment = copy.deepcopy(static_covariate_encoder) # 用来增加语义
        self.static_encoder_sequential_cell_init = copy.deepcopy(static_covariate_encoder) # 输给LSTM
        self.static_encoder_sequential_state_init = copy.deepcopy(static_covariate_encoder) # 输给LSTM
        ...
        # ============================================================ 
        # GAT processing of the od-wise context
        # ============================================================
        self.past_gat = GAT(input_dim=2*self.state_size,
                            output_dim = 2*self.state_size,
                            num_heads=self.attention_heads,
                            dropout=self.dropout)
                            
        self.future_gat = GAT(input_dim=2*self.state_size,
                              output_dim = 2*self.state_size,
                              num_heads=self.attention_heads,
                              dropout=self.dropout)
        
        # ============================================================
        # Static enrichment: 带静态语义的GRN模块
        # ============================================================
    ...
    def apply_temporal_selection(temporal_representation: torch.tensor,
        ...
        time_distributed_context = TFT_GAT.replicate_along_time(static_signal=static_selection_signal,
        ...
        temporal_flattened_embedding = TFT_GAT.stack_time_steps_along_batch(temporal_representation)
        time_distributed_context = TFT_GAT.stack_time_steps_along_batch(time_distributed_context)
        # Dimensions:
    ...
    def apply_sequential_processing(self, lstm_model: nn.LSTM, selected_feature: torch.tensor, 
        ...
        # hidden_bridge: (h,c), h or c: [lstm_layers x num_samples x state_size]
        
        lstm_bridge, hidden_bridge = lstm_model(selected_feature,hidden)
        h_tensor, c_tensor = hidden_bridge[0][-1].squeeze(0), hidden_bridge[1][-1].squeeze(0)
        
        combined_hc_bridge = torch.cat([h_tensor, c_tensor], dim=-1)
        
        return lstm_bridge, combined_hc_bridge
    ...
    def apply_self_attention(self, enriched_sequence: torch.tensor,
        ...
        if self.is_mask: # 如果有mask就会保证因果关联, 也就是每个未来步只能从之前的步骤获取信息
    ...
    def forward(self, batch, edge_index, target_N, IS_EVAL, IS_STATION, IS_PHYSIC):
        ...
        # =========== Locality Enhancement - Sequential Processing + Neighbor Aggregation ==============
        # Past LSTM
        static_hidden = (c_seq_hidden.unsqueeze(0).repeat(self.lstm_layers, 1, 1),
                         c_seq_cell.unsqueeze(0).repeat(self.lstm_layers, 1, 1))
        past_lstm_bridge, past_combined_hc_bridge  = self.apply_sequential_processing(lstm_model = self.past_lstm,
                                                                                      selected_feature=selected_historical,
                                                                                      hidden=static_hidden)
        
        # Past GAT on Past Context
        if edge_index is not  None:
            past_combined_hc_gat, past_edgeindex, past_attention_alpha = self.past_gat(past_combined_hc_bridge, edge_index)
            past_combined_hc_gat = past_combined_hc_gat.unsqueeze(0).repeat(self.lstm_layers, 1, 1).contiguous()
            past_combined_hc_gat = torch.chunk(past_combined_hc_gat, 2, dim=-1)
            past_combined_hc_gat = tuple(h.contiguous() for h in past_combined_hc_gat)

        # Past LSTM with Neighbor Info
            past_lstm_output, hidden_past  = self.apply_sequential_processing(lstm_model = self.past_lstm,
                                                                              selected_feature = past_lstm_bridge,
                                                                              hidden = past_combined_hc_gat)

        else:
            past_lstm_output, hidden_past = past_lstm_bridge, past_combined_hc_bridge
        
        hidden_past = hidden_past.unsqueeze(0).repeat(self.lstm_layers, 1, 1).contiguous()
        hidden_past = torch.chunk(hidden_past, 2, dim=-1)
        hidden_past = tuple(h.contiguous() for h in hidden_past)
        
        # Future Lstm
        future_lstm_bridge, future_combined_hc_bridge = self.apply_sequential_processing(lstm_model = self.future_lstm,
                                                                          selected_feature = selected_future,
                                                                          hidden = hidden_past)
        
        # Future GAT on Future Context
        if edge_index is not  None:
            future_combined_hc_gat, future_edgeindex, future_attention_alpha = self.future_gat(future_combined_hc_bridge, edge_index)
            future_combined_hc_gat = future_combined_hc_gat.unsqueeze(0).repeat(self.lstm_layers, 1, 1).contiguous()
            future_combined_hc_gat = torch.chunk(future_combined_hc_gat, 2, dim=-1)
            future_combined_hc_gat = tuple(h.contiguous() for h in future_combined_hc_gat)
            
            # Future LSTM with Neighbor Info
            future_lstm_output, _ = self.apply_sequential_processing(lstm_model = self.future_lstm,
                                                                     selected_feature = future_lstm_bridge,
                                                                     hidden = future_combined_hc_gat)
        else: 
            future_lstm_output = future_lstm_bridge

        # concatenate the historical spatiotemporal output with the futuristic spatiotemporal output, along the time dimension
        lstm_input = torch.cat([selected_historical[:target_N], selected_future[:target_N]], dim=1)
        lstm_output = torch.cat([past_lstm_output[:target_N], future_lstm_output[:target_N]], dim=1)
        ...
        # =========== Static enrichment ==============
        c_enrichment = c_enrichment[:target_N]
        ...
        gated_poswise_ff = self.pos_wise_ff_gating(
            post_poswise_ff_grn,
            residual=gated_lstm_output[:, (self.num_historical_steps + self.target_window_start_idx):, :])
        # Dimensions:
        ...
        predicted_quantiles_IU = self.output_layer_IU(gated_poswise_ff)
        
        # =========== output projection for R (PINN) =============
        # 样本编号
        batch_indices = batch['ODindex'][:target_N]

        # 主信号流：出行时间的信息,重新嵌入
        TT_ts = batch['future_ts_numeric'][:target_N,:,self.TT_ts_index]   # 1/TT为0时, S一定为0, Rb一定为无穷大
        SL_ts = batch['future_ts_numeric'][:target_N,:,self.SL_ts_index]   # 1/TT不为0时, 且SL为0时, Rb一定为1, S为最大值
        ...
        W_ts_rep = future_ts_rep[:target_N, :, W_idx_tensor] # [num_samples x num_future_steps x (total_num_W_inputs * state_size)]
        
        # W_ts = batch['future_ts_categorical'][:target_N,:,self.W_ts_index]
        ...
        # O_raw = torch.cat([batch['static_feats_categorical'][:target_N,self.O_index_categorical],batch['static_feats_numeric'][:target_N,self.O_index_numeric]],dim=-1)
        ...
        # OD_raw = torch.cat([batch['static_feats_categorical'][:target_N,self.OD_index_categorical],batch['static_feats_numeric'][:target_N,self.OD_index_numeric]],dim=-1)
        ...
        Hour_ts_rep = future_ts_rep[:target_N, :, Hour_idx_tensor] # [num_samples x num_future_steps x (total_num_Hour_inputs * state_size)]
        ...
        # Day_ts = batch['static_feats_categorical'][:target_N,self.Day_index]
        ...
        phi, s, alpha, beta, eta, rb, r_physics, congestion = self.pinn(
            ...
            phi_raw = phi_ts,
            maxphi = maxphi,
            W = W_ts_rep,#  W_ts.float(),
            Hour = Hour_ts_rep, #  Hour_ts.float(),
            Hour_raw = Hour_ts,
            Day = Day_rep, #  Day_ts.float(),
            O =  O_rep, #  O_raw.float(),   # 注意：这里传静态的，LSIM内部会处理上下文逻辑
            OD = OD_rep, #  OD_raw.float(),
            batch_indices = batch_indices,
            IS_EVAL = IS_EVAL
        )

        return {
            'predicted_quantiles_IU': predicted_quantiles_IU,  # [num_samples x output_sequence_length x num_quantiles]
            ...
            'attention_scores': attention_scores, # [num_samples x output_sequence_length x (num_historical_steps + num_future_steps)]
            'static_weights': static_weights.squeeze(-1), # [num_samples x num_static_inputs]
            'historical_selection_weights': historical_selection_weights, # [num_samples x num_historical_steps x total_num_historical_inputs]
            'future_selection_weights': future_selection_weights, # [num_samples x num_future_steps x total_num_future_inputs]
